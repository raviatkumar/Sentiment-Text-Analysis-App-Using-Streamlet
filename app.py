...
def predict_emotions(docx):
	results = pipe_logic_regression.predict([docx])
	return results[0]

def get_prediction_proba(docx):
	results = pipe_logic_regression.predict_proba([docx])
	return results

emotions_emoji_dict = {"anger" : "😠", "disgust" : "🤮", "fear" : "😨", "happy" : "🤗", "joy" : "😂", "neutral" : "😐", "sad" : "😔", "shame" : "😳", "surprise" : "😮"}
...
def main():
	...
	if choice == "Home":
		...
		st.subheader("Emotion Detection Text")
		...
		if submit_text:
			...
			with col2:
				...
				proba_df = pd.DataFrame(probability, columns = pipe_logic_regression.classes_)
				# st.write(proba_df.T)
